use crate::embeddings::embedding_model_service::{
    EmbeddingModelApiAsync, EmbeddingModelService, EmbeddingService,
};
...
impl CreatorFactory {
        ...
        match self.embedding_model.get_model_provider() {
            ModelProvider::OpenAi => {
                ...
                return OpenaiEmbeddings::new(
                    self.openai_api_key.as_ref().expect("empty openai key"),
    ...
    pub fn create_embedding_service(
        &self,
    ) -> EmbeddingModelService<impl EmbeddingModelApiAsync, impl EmbeddingMemory> {
