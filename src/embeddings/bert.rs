...
pub fn get_embeddings(sentence: &str, model: &BertModel, tokenizer: &Tokenizer) -> Result<Vec<f32>> {
    ...
    let vec2 = embeddings.to_vec2().context("Unable to get embeddings to vec1")?;
