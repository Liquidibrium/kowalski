use crate::embeddings::bert::{EmbeddingModelConfig, get_embeddings, load_model_bert_model, ModelAndTokenizer};

pub trait EmbeddingCalculator {
    fn new(model: &String) -> Self;
    fn get_embedding(&self, code: &str) -> Vec<f32>;
}
...
impl EmbeddingCalculator for EmbeddingModelLocal {
    ...
    fn get_embedding(&self, code: &str) -> Vec<f32> {
        return get_embeddings(code, &self.model.model, &self.model.tokenizer).expect("Failed to get embeddings");
    }
}
