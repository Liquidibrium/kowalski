...
export default function SignUp() {
  ...
  const onEmailInputChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    setEmail(event.target.value.trim());
    setError(undefined);
  }, []);

  const onPasswordInputChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    setPassword(event.target.value.trim()) ;
    setError(undefined);
  }, []);
  const onFullNameInputChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    setFullName(event.target.value);
    setError(undefined);
  }, []);


  return <div className="shadow-three mx-auto max-w-[500px] rounded bg-white px-6 py-10 dark:bg-dark sm:p-[60px]">
